...
function EWS(config, options) {
  ...
  if(configIsValid(config)) {
    ...
    // ntlm auth
    var ntlmAuth = {
      wsdlOptions: _.merge({httpClient: HttpClient}, _.clone(options)),
        ...
        return when.promise((resolve, reject) => {
          request(requestOptions, function(err, res, body) {
            if(err) {
              ...
              reject(new Error('Basic Auth StatusCode 401: Unauthorized.'));
            } else {
      ...
      return when.promise((resolve, reject) => {
        ...
        soap.createClient(wsdlFilePath, wsdlOptions, (err, client) => {
          if (err) {
            ...
            if (client[ewsFunction] === undefined) {
              throw new Error('Function ' + ewsFunction + ' is undefined, wsdl download probably failed');
            }
